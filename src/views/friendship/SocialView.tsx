...
type FilterType = 'all' | 'add' | 'remove' | 'name_change' | 'rank_change' | 'bio_change';
...
export const SocialView: React.FC = () => {
    ...
    const fetchEvents = useCallback(async () => {
        ...
        try {
            const data = await window.electron.friendship.getRelationshipEvents();
    ...
    const filteredEvents = events.filter(e => {
        if (e.type === 'avatar_change') return false;
    ...
    return (
        <GlassPanel style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            {/* Table Coiainer - Uses Absolute Fill to force scroll within available space (COPIED FROM GAMELOGVIEW) */}
